...
function get_test_details($pdo, $user_id) {
    ...
    $stmt = $pdo->prepare("SELECT id FROM test_attempts WHERE user_id = ? AND test_id = ? AND status = 'completed'");
    $stmt->execute([$user_id, $test_id]);
    if ($stmt->fetch()) {
        json_response(false, "You have already completed this test.");
    }

    // Check for an existing INCOMPLETE attempt to resume
    $stmt = $pdo->prepare("SELECT id FROM test_attempts WHERE user_id = ? AND test_id = ? AND status = 'started'");
    $stmt->execute([$user_id, $test_id]);
    $existing_attempt = $stmt->fetch(PDO::FETCH_ASSOC);

    $attempt_id = $existing_attempt ? $existing_attempt['id'] : null;
    ...
    if (!$attempt_id) {
        $stmt = $pdo->prepare("INSERT INTO test_attempts (user_id, test_id, status) VALUES (?, ?, 'started')");
        $stmt->execute([$user_id, $test_id]);
...
function submit_test($pdo, $user_id) {
    ...
    try {
        // Clear previous answers for this attempt to prevent duplicates on resume
        $stmt_delete = $pdo->prepare("DELETE FROM answers WHERE attempt_id = ?");
        ...
        $percentage = ($total_test_marks > 0) ? round(($total_marks_obtained / $total_test_marks) * 100, 2) : 0;

        // Use INSERT ... ON DUPLICATE KEY UPDATE to avoid creating a new result for a resumed test
        $sql_result = "
            INSERT INTO results (attempt_id, user_id, test_id, marks_obtained, total_marks, percentage, status, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, NOW())
